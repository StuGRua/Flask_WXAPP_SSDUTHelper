...
def new_user():
    ...
    if User.query.filter_by(userid=userid).first() is not None or User.query.filter_by(phonenumber=phonenumber).first() is not None:
        abort(400)  # existing user
    user = User(userid=userid,phonenumber=phonenumber,user_name='xxx')
